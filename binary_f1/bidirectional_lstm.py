...
def main():
    ...
    results_file.write(Metrics.get_csv_header('bi-lstm', 'binary') + '\n')
    lstm_cv_results = []
    lstm_csv_logger = CSVLogger(f'results/bilstm/lstm_history.csv',
                                append=True, separator=',')
    for train, test in strat_kfold.split(x, y):
        model = build_lstm_model(len(vocab), vocab)
        model.fit(x[train], y[train], hps['batch_size'],
                  epochs=hps['epochs'], callbacks=[lstm_csv_logger])
        _, accuracy, precision, recall, f1 = \
            model.evaluate(x[test], y[test], hps['batch_size'])
    ...
    results_file.write(Metrics.get_csv_header('bi-gru', 'binary') + '\n')
    gru_cv_results = []
    gru_csv_logger = CSVLogger(f'results/bilstm/gru_history.csv',
                               append=True, separator=',')
    for train, test in strat_kfold.split(x, y):
        model = build_gru_model(len(vocab), vocab)
        model.fit(x[train], y[train], hps['batch_size'],
                  epochs=hps['epochs'], callbacks=[gru_csv_logger])
        _, accuracy, precision, recall, f1 = \
            model.evaluate(x[test], y[test], hps['batch_size'])
...
def build_lstm_model(vocab_size, pretrained_weights):
    ...
    model.add(Bidirectional(LSTM(units=hps['pad_length'],
                                 dropout=hps['dropout'],
...
def build_gru_model(vocab_size, pretrained_weights):
    ...
    model.add(Bidirectional(
        GRU(units=hps['pad_length'], dropout=hps['dropout'],
            recurrent_dropout=hps['recurrent_dropout'])))
    model.add(Dense(1, activation='sigmoid'))
    model.compile(loss='binary_crossentropy',
                  optimizer=optimizers.Adam(hps['lr']),
